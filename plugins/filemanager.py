...
@app.on_message(
    filters.command("dn") & ~filters.forwarded & ~filters.via_bot & SUDOERS, group=88
)
async def download_file(_, message):
    """
    Download a file from the `/home/administrator` directory or a given path.
    """
    base_directory = "/home/administrator"
    text = message.text.split(" ", 1)

    if len(text) < 2:
        await message.reply("Please specify the file name or path to download.")
        return
    ...
    if os.path.exists(file_path) and os.path.isfile(file_path):
        if os.path.getsize(file_path) > 50 * 1024 * 1024:  # 50 MB size limit
            await message.reply("The file size exceeds the 50 MB limit.")
            return
