    ...
    try:
        ...
        if not userbot:
            return await message.reply_text(
                "Userbot assistant not found or unable to promote."
            )
        ...
        try:
            ...
            ok = await message.reply_text(
                "Userbot has been promoted to admin with delete messages permission."
            )
            await ok.delete()
        ...
        if len(message_ids) > 0:
            try:
                ...
                return await message.reply_text(
                    f"Failed to delete remaining messages: {str(e)}"
                )
...
@app.on_message(
    filters.command(
        ["deleteallgroup", "deleteallgroupmsg", "delallgroupmessage", "cleangroupmsg"]
    )
    & filters.group
)
    ...
    try:
        if not await is_admin_or_sudo(client, message.chat.id, message.from_user.id):
            await message.reply_text(
                "๏ ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴛᴏ ᴘᴇʀғᴏʀᴍ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ."
            )
            return
            ...
            [
                [
                    InlineKeyboardButton(
                        "ᴄᴏɴғɪʀᴍ ᴅᴇʟᴇᴛɪᴏɴ",
                        callback_data=f"confirm_delete_group:{chat_id}",
                    )
                ],
        ...
        await message.reply_text(
            "๏ ᴀʀᴇ ʏᴏᴜ sᴜʀᴇ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀʟʟ ᴍᴇssᴀɢᴇs ғʀᴏᴍ ᴛʜᴇ ɢʀᴏᴜᴘ?",
    ...
    try:
        ...
        if not await is_admin_or_sudo(client, chat_id, callback_query.from_user.id):
            await callback_query.answer(
                "๏ ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴛᴏ ᴜsᴇ ᴛʜɪs ʙᴜᴛᴛᴏɴ.", show_alert=True
            )
            return
